...
<div class="title-dashboard-2">
    ...
    <form action="/dashboard/book/{{ $Book->slug }}" method="POST"">
        ...
        <input type="text" id="nama_buku" name="nama_buku" value="{{ old('nama_buku', $Book->nama_buku) }}">
        @error('nama_buku')
      ...
      @enderror
        ...
        <label>slug</label>
        <input type="text" id="slug" name="slug" value="{{ old('slug', $Book->slug) }}">
        @error('slug')
        ...
        <select name="category_id">
         ...
         <p style="color: red">Silahkan Isi!</p>   
       @enderror
        <input id="isi" type="hidden" name="isi" value="{{ old('isi', $Book->isi) }}">
